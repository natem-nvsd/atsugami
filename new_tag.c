#include "error_dialogs.h"
#include <glib/gstdio.h>
#include <gtk/gtk.h>
#include "import.h"
#include <libpq-fe.h>
#include "main.h"
#include "new.h"
#include <stdio.h>

PGresult *tagres;
GtkWidget *tag_dialog, *tag_entry;

static void on_tag_apply(GtkWidget *widget, gpointer data) {
	//char psql_error[2048];
	const gchar *text;
	int query_size = (sizeof(text) + 70);

	char query_base[] = "INSERT INTO public.tags (name) VALUES ('";
	char tag_query[query_size];

	strcpy(tag_query, query_base);

	text = gtk_entry_get_text(GTK_ENTRY(tag_entry));
	/*
	if (gtk_entry_get_text_length(GTK_ENTRY(tag_entry) == 0)) {
		fprintf(stderr, "This cannot be null\n");		// add an info bar here
	} */
	//if (gtk_entry_get_text_length(GTK_ENTRY(tag_entry) > 0)) {
		strcat(tag_query, text);
	//}

	strcat(tag_query, "') ON CONFLICT DO NOTHING;");
	PQexec(conn, tag_query);
	strcpy(tag_query, "");	// clear the query string

	// the query is done, now the window must be destroyed
	gtk_widget_destroy(tag_dialog);
}

static void on_tag_cancel(GtkWidget *widget, gpointer data) {
	gtk_widget_destroy(tag_dialog);
}

/* Quick and dirty solution */
extern void new_tag_activate(void) {
	GtkWidget *label, *button0, *button1, *vbox, *bbox;

	//tag_dialog = gtk_window_new(GTK_WINDOW_POPUP);
	tag_dialog = gtk_window_new(GTK_WINDOW_TOPLEVEL);

	vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 10);
	gtk_container_set_border_width(GTK_CONTAINER(vbox), 10);
	gtk_container_add(GTK_CONTAINER(tag_dialog), vbox);

	/* label */
	label = gtk_label_new("Enter the tag's name here:");
	gtk_widget_set_valign(label, GTK_ALIGN_START);
	gtk_box_pack_start(GTK_BOX(vbox), label, FALSE, FALSE, 0);

	/* entry */
	tag_entry = gtk_entry_new();
	gtk_entry_set_activates_default(GTK_ENTRY(tag_entry), TRUE);
	gtk_widget_set_valign(tag_entry, GTK_ALIGN_START);
	gtk_box_pack_start(GTK_BOX(vbox), tag_entry, TRUE, TRUE, 0);

	/* bbox */		// replace with gtk button box
	bbox = gtk_button_box_new(GTK_ORIENTATION_HORIZONTAL);
	gtk_container_add(GTK_CONTAINER(vbox), bbox);

	/* buttons */
	button0 = gtk_button_new();
	gtk_box_pack_start(GTK_BOX(bbox), button0, TRUE, TRUE, 0);
	gtk_button_set_label(button0, "Cancel");
	g_signal_connect(button0, "clicked", G_CALLBACK(on_tag_cancel), NULL);

	button1 = gtk_button_new();
	gtk_box_pack_start(GTK_BOX(bbox), button1, TRUE, TRUE, 0);
	gtk_button_set_label(button1, "Add");
	g_signal_connect(button1, "clicked", G_CALLBACK(on_tag_apply), NULL);

	/* SHow the window */
	gtk_window_set_resizable(tag_dialog, FALSE);
	gtk_window_set_position(tag_dialog, GTK_WIN_POS_CENTER_ALWAYS);
	gtk_window_set_default_size(GTK_WINDOW(tag_dialog), 300, 100);
	gtk_window_set_title(tag_dialog, "Add tag");
	gtk_widget_show_all(vbox);
	gtk_widget_show_all(tag_dialog);
}
