...
static int on_tag_apply(GtkWidget *widget, gpointer data) {
	...
	sprintf(tag_query, "INSERT INTO public.tags_categories (tag_id, category_id) VALUES (%s, 3);", PQgetvalue(tagres, 0, 0));
	PQclear(tagres);
...
extern void new_tag_activate(void) {
	...
	gtk_notebook_set_tab_label_text(GTK_NOTEBOOK(notebook), tag_vbox, "New tag");
