...
static void expander_cb(GtkExpander *expander, GParamSpec *pspec, GtkWindow *dialog) {
	gtk_window_set_resizable (dialog, gtk_expander_get_expanded (expander));
}

static void do_not_expand(GtkWidget *child, gpointer data) {
	gtk_container_child_set(GTK_CONTAINER(gtk_widget_get_parent(child)), child, "expand", FALSE, "fill", FALSE, NULL);
}
...
extern GtkWidget *error_activate(GtkWidget *do_widget) {
	...
	if (!window) {
		...
		window = gtk_message_dialog_new_with_markup(GTK_WINDOW(toplevel), 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "<big><b>%s</b></big>", "This feature has not been implemented yet.");
		gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(window), "Here are some more details " "but not the full story.");

	area = gtk_message_dialog_get_message_area(GTK_MESSAGE_DIALOG(window));
	box = gtk_widget_get_parent(area);
	gtk_container_child_set(GTK_CONTAINER(gtk_widget_get_parent(box)), box, "expand", TRUE, "fill", TRUE, NULL);
...
/*                              "Finally, the full story with all details. "
	...
	g_signal_connect(expander, "notify::expanded", G_CALLBACK(expander_cb), window);
...
extern GtkWidget *postgres_error_activate(GtkWidget *do_widget) {
	...
	if (!window) {
		toplevel = gtk_widget_get_toplevel(do_widget);
		window = gtk_message_dialog_new_with_markup(GTK_WINDOW(toplevel), 0, GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE, "<big><b>%s</b></big>", "PostgreSQL error.");
		gtk_message_dialog_format_secondary_text(GTK_MESSAGE_DIALOG(window), "Click for more details.");
